...
from dimstim.Core import sec2intvsync, deg2pix, pix2deg, iterable, isotime, intround, roundec
...
class ManBar(Experiment):
    ...
    def on_key_press(self, symbol, modifiers):
        if symbol == key.UP:
            ...
            self.MINUS = True
        elif symbol == key.BRACKETLEFT: #DW
            self.BRLEFT = True
        elif symbol == key.BRACKETRIGHT:
            self.BRRIGHT = True
        elif symbol == key.COMMA: #cycles through types of flashes
            self.flashType += 1
            if self.flashType > 3:
                self.flashType = 1
    ...
    # Code added by Derric AIBS
    def check_input(self): #can be any type of input
        pass
        ...
        while np.alltrue([ not win.win.has_exit for win in self.wins ]):
            ...
            if self.flash:
                if not self.nvsyncsdisplayed % self.flashvsyncs:
                    ...
                    elif self.flashType == 2:
                        self.brightness,self.black = self.black, self.brightness
                    elif self.flashType == 3:
                        self.brightness, self.white = self.white, self.brightness
