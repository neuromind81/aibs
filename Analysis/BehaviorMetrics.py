...
def BehaviorMetrics(logpath):
    ...
    figtext(0.1, 0.05, startdatetime)
    figtext(0.1, 0.1, "# false positives / percent incorrect:")
    figtext(0.5, 0.1, str(falsepositive) + " / " + str(percentfp))
    figtext(0.1, 0.15, "# rewards / percent correct:")
    figtext(0.5, 0.15, str(numberrewards) + " / " + str(percentcorrect))
    figtext(0.1, 0.2, "total distance run(cm) / water (mL):")
    figtext(0.5, 0.2, str(totaldistance) + " / " + str(totalwater))
