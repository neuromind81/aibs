...
['Name',[PossibleValues],[CorrectValues],RelevanceBool]
...
t = Terrain()
t.params.append(['Color',[-1,0,1],[1],True])            #set up color
t.params.append(['Ori',range(0,360,45),[0,45],True])    #set up orientation
...
class Terrain(object):
    ...
    def __init__(self, params = []):
        ...
        #Format for values ['Color',[PossibleValues],[CorrectValues],[RelevanceBool]]
        #self.params.append(['Color',[-1,0,1],[1],True])
    ...
    def correct(self):
        ...
        for p in self.params:
            if p[3]:
                self.current.append(random.choice(p[2]))
            else:
                self.current.append(random.choice(p[1]))
        self.iscorrect = True
    ...
    def incorrect(self):
        ...
        for i in range(len(self.params)):
            if (self.current[i] in self.params[i][2]) and self.params[i][3]:
                flag +=1
            if self.params[i][3]:
                rel +=1
        if flag == rel:
...
if __name__ == "__main__":
    t = Terrain()
    t.params.append(['Color',[-1,0,1],[1],True])
    t.params.append(['Ori',range(0,360,45),[0,45],True])
    t.current = [-1,45]
    print t.params
    for i in range(100):
        print t.iscorrect
